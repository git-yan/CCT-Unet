...
class CCT_Unet(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=96, depth=[2, 2, 6, 2],
        ...
        self.stage_conv_embed = nn.Sequential(
            nn.Conv2d(3, embed_dim // 2, kernel_size=3, stride=2, padding=1, bias=True),
            nn.BatchNorm2d(embed_dim // 2, eps=1e-5),
            nn.GELU(),
        ...
        self.merge0 = Merge_Block(embed_dim//2, embed_dim)
    ...
    def load_from(self, pretrained_path, models):
        if pretrained_path is not None:
            ...
            print("---finish load pretrained modle of CCT_Unet encoder---")
            # print(msg)
        else:
            print("none pretrain")
